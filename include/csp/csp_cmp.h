...
static inline int csp_cmp_##_memb(struct request_metadata_t *metadata, uint16_t node, uint32_t timeout, struct csp_cmp_message *msg) { \
	return csp_cmp(metadata, node, timeout, _code, CMP_SIZE(_memb), msg); \
}
...
/**
   ...
   @return #CSP_ERR_NONE on success, otherwise an error code.
*/
static inline int csp_cmp_peek(struct request_metadata_t *metadata, uint16_t node, uint32_t timeout, struct csp_cmp_message *msg) {
    return csp_cmp(metadata, node, timeout, CSP_CMP_PEEK, CMP_SIZE(peek) - sizeof(msg->peek.data) + msg->peek.len, msg);
}
...
/**
   ...
   @return #CSP_ERR_NONE on success, otherwise an error code.
*/
static inline int csp_cmp_poke(struct request_metadata_t *metadata, uint16_t node, uint32_t timeout, struct csp_cmp_message *msg) {
    return csp_cmp(metadata, node, timeout, CSP_CMP_POKE, CMP_SIZE(poke) - sizeof(msg->poke.data) + msg->poke.len, msg);
}
