...
csp_conn_t * csp_connect(uint8_t prio, struct request_metadata_t *metadata, uint16_t dest, uint8_t dport, uint32_t timeout, uint32_t opts) {
	...
	incoming_id.dst = metadata->return_node <= 0 ? csp_conf.address : metadata->return_node;
	incoming_id.src = dest;
	incoming_id.sport = dport;
	incoming_id.flags = 0;
	outgoing_id.pri = prio;
	outgoing_id.dst = dest;
	outgoing_id.src = metadata->return_node <= 0 ? csp_conf.address : metadata->return_node;
